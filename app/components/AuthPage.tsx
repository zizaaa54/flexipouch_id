...
export default function AuthPage({ onLogin }: { onLogin: () => void }) {
  ...
  const db = getFirestore(app); // 🔥 TAMBAHAN
  ...
  const handleRegister = async () => {
    try {
      ...
      await setDoc(doc(db, "user", user.uid), {
        email: user.email,
        role: "admin", // 🔥 ubah ke "user" kalau bukan admin
      });
  ...
  return (
    <div className="w-screen h-screen flex bg-[#f5f7f7] overflow-hidden">
      ...
      <motion.div
        ...
        className="absolute top-0 left-1/2 w-1/2 h-full bg-[#8FAEB0] flex items-center justify-center text-center"
      >
